...
class ScreenshotHelper {
    ...
    async fullPage(path, type) {
        ...
        // const bh = await this.page.$('.xly__browser__header');
        // if (!bh) {
        //     return false;
        // }
        ...
        let isHeaderPainted = new Promise((resolve) => {
            loadHeaderTimer = setInterval(async () => {
                ...
                //     const dom = document.querySelector('.xly__browser__header');
                //     return !!dom;
                // })
                // if (dom) {
                ...
                try {
                    ...
                    if (nodeOfLocation && nodeOfLocation.backendNodeId) {
                        ...
                        const isRightPositin = outerHtml.outerHTML.indexOf('<div class="xly__browser__header">') !== -1;
                        if (isRightPositin) {
                            clearInterval(loadHeaderTimer);
                            clearTimeout(loadHeaderTimeout);
                            resolve(true);
